...
out <- rbind(out, data.frame(analysis = c("betadispersion", "adonis", "cca"), 
                             data = d, 
...
```{r}
flextable(out %>% arrange(analysis))
```
...
hdpi95 <- res %>% group_by(model, parameter, status_dietmatch) %>% 
  summarise(median = median(value), lower = hdi(value, 0.95)$CI_low, upper = hdi(value, 0.95)$CI_high) %>% 
  ...
  mutate(parameter = recode(parameter, omega = "theta", sigma = "variance"), 
         ...
         status = recode(status, intact = "Intact")) %>% 
