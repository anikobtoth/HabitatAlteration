...
\[
p(\sigma_g) = \frac{1}{\Gamma(2)0.5^2} \sigma_g e^{-\sigma_g/0.5} = 4\sigma_g e^{-\sigma_g/0.5}
\]
...
__Table S1.__ 
Median values and 95% high density posterior intervals for  $\psi$ and $\sigma_{g}$ of all bat and bird models.
```{r echo=FALSE, message=FALSE, warning=FALSE}
library(flextable)
...
hdpi95 <- res %>% group_by(model, parameter, status_dietmatch) %>% 
  summarise(median = median(value), lower = hdi(value)[1], upper = hdi(value)[2]) %>% 
  separate(status_dietmatch, into = c("status", "Pair_type")) %>%
  separate(model, into = c("taxon", "model"), extra = "merge") %>%
  mutate(parameter = recode(parameter, omega = "psi", sigma = "variance"), 
 ...
 pivot_wider(names_from = parameter, values_from = c(median, lower, upper), names_sort = TRUE) %>%
  select(taxon, model, status, Pair_type, ends_with("psi"), ends_with("variance"))
  
flextable(hdpi95) %>% 
 set_header_labels(median_psi = "Median ψ", 
                   lower_psi = "Lower ψ", 
                   upper_psi = "Upper ψ", 
