...
__Figure 3.__ Posterior distributions of $\bar{\theta_{g}}$ for bat and bird models, including both those that contained all species (full models) and those containing only species that occurred in both altered and intact habitat types (no-turnover models).
...
Effects of species occurrence (indexed by $N_{A}$ and $N_{B}$) and spatial aggregation ($\theta > 0$) and segregation ($\theta < 0$) on the probability distribution of co-occurrences. Probabilities were calculated using Eq. 1 based on a total of $N_{A} + N_{(A)}=50$ sites.
