...
As important drivers of community structure, interactions help determine how species assemble themselves across landscapes, maintain biodiversity, and are crucial to the provision of many ecosystem services such as pollination, seed dispersal, and pest control. Anthropogenic disturbance puts the natural course of these processes in jeopardy. 
...
Here we show that human habitat alteration is associated with a decrease in the spatial aggregation of Neotropical bat pairs and bird pairs in the same dietary guilds.
We found that all groups of species pairs tested have positive co-occurrence on average, but pairs within dietary guilds have stronger co-occurrence than pairs with disparate diets when habitats are relatively intact.
...
__Figure 1.__ Bird and bat community structure. Multidimensional scaling analysis of altered and intact sites for bats (A) and birds (B) based on the Ochiai dissimilarity index [@Hubalek1982]. 

![](../../Figures/Fig2_concept_v2.png) 

__Figure 2.__ Conceptual figure demonstrating the experimental design. (A) Pairwise combinations of species shown as an adjacency matrix, where black squares represent granivores and yellow squares represent nectarivores. White numbers indicate species occupancy. Pairs inside the matrix are placed in groups (g) as “competing” or “control” according to shared or disparate guild assignment, respectively. Black numbers represent the occupancy pattern of the pair. (B) A distribution of 𝜃 is calculated for sets of pairs N~AB~ based on occupancy pattern. Coloured squares represent cells from the adjacency matrix, i.e. species pairs. From these distributions, (C) the cumulative posterior distribution of each group mean ¯𝜃𝑔 is derived and compared. This process is done separately for altered and intact sites, then compared. 

```{r echo=FALSE, fig.height=4, fig.width=3, message=FALSE, warning=FALSE, include=T}
...
__Figure 3.__ $\bar{\theta{g}}$ posteriors for bat models. Models containing (A) all bats, and (B) bats that occurred in both intact and altered habitats (no turnover models).

```{r echo=FALSE, fig.height=4, fig.width=3, include = T, warning = FALSE, message = FALSE}
...
__Figure 4.__ $\bar{\theta{g}}$ posteriors distributions for bird models. Models containing (A) all birds, and (B) birds that occurred in both intact and altered habitats (no turnover models).
...
for the _score_, defined as the gradient of the log-likelihood in relation
...
the Fisher Information, which is the variance of the score [@Ly2017a]. We
...
(ii)  A _single_ observation of a non-overlapping distribution will not 
...
\[
p(\boldsymbol{N_{AB}} \mid \theta_i) = \prod_{j \in \mathcal{S}_i} p(N_{AB}^j|\theta_i)
\]
...
(*iii*) because the Fisher Information is additive [@Ly2017a],
...
__Extended Data Fig. 2__ 
Effects of taxon occurrence (indexed by $N_{A}$ and $N_{B}$) and spatial aggregation ($\theta > 0$) and segregation ($\theta < 0$) on the probability distribution of co-occurrences. Probabilities were calculated using Eq. (S1) based on a total of $N_{A} + N_{(A)}=50$ sites.
...
Effects of occurrence ($N_{A}$ and $N_{B}$) and spatial aggregation ($\theta$) on the probability distribution of co-occurrences. Probabilities were calculated using Eq. (S3), assuming a total of $N_{A} + N_{(A)}=50$ sites.
