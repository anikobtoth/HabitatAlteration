...
*__Proportion of pairs aggregated.__* The main text of this study presents results on the strength of aggregations and segregations in competing and non-competing pairs, and how this changes between altered and unaltered habitats. To understand these patterns in more detail, we present the same analysis using proportion of pairs aggregated as the measurement variable. Although a few pairs may come up as exactly zero, the vast majority of pairs will be either positive or negative, so the proportion of segregations will be directly inversely proportional to aggregations. As such, presenting both is unnecessary. 
...
load("../Data/original_coords.Rdata")
...
```{r, echo = F, message = FALSE, include = TRUE, fig.width = 9, fig.height=4, warning=FALSE}
d <- d5.prop.catp
